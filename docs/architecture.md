...
- All fetch paths degrade, never throw to the user: history → trades ∪ candles → `[]` (`app/api/history/route.ts:88-110`); price → Coinbase → Binance → 502 (`app/api/price/route.ts:23-39`); settle → live tick → REST (`app/page.tsx:59-64`).
- `PriceChart` must stay a pure function of props — including its clock, which arrives as the `now` prop rather than a `Date.now()` read or an interval of its own (`app/page.tsx:226`). Round/freeze logic belongs in `page.tsx` (`app/page.tsx:221-227`), not the chart. Same for lock timestamps: `page.tsx` stamps `lockedAt{1,2}`, the chart just draws whatever `PredictionLine.at` it gets.
