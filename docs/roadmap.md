...
| 24h Reading | UI only, BTC-only: `app/Battle24h.tsx` renders inline in the `/duel` lobby by default (no separate Play step), and is reused as-is by the standalone deep link `app/duel/btc/battle/page.tsx` — pick/vote-split/streak/calendar all client `useState`, no schema, no settlement job, no cron. Identity and day-boundary (UTC midnight, decided) still need a real per-player store before this is more than a mock |
