...
function Score({ name, equity, pnl, align = "left" }: { name: string; equity: number; pnl: number; align?: "left" | "right" }) {
  return <div className={`min-w-0 ${align === "right" ? "text-right" : ""}`}><span className="truncate text-xs uppercase tracking-wider text-[var(--muted)]">{name}</span><div className="mt-1 text-lg font-semibold tabular-nums text-[var(--text)]">{usd(Math.max(0, equity))}</div><div className="mt-0.5 text-xs font-medium tabular-nums" style={{ color: pnlColor(pnl) }}>{signedUsd(pnl)} P&amp;L</div></div>;
}
